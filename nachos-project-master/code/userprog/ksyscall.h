...
void SysHalt() { 
	kernel->currentThread->start_time = clock()-(kernel->currentThread->start_time);
	float time = float(kernel->currentThread->start_time)/CLOCKS_PER_SEC;
...
//printf("jhvgjgjidrghktrudrfgvhgkjytrdfcgvhytrdfcgvhgytrfg");
...
void SysPrintString(char* buffer, int length) {
    for (int i = 0; i < length; i++) {
   if(buffer[i] == ' ') buffer[i]='#';  /* code added by me */
